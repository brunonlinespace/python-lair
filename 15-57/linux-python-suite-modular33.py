...
class LinuxPythonLauncherEditor(QMainWindow):
    def __init__(self):
        ...
        self.current_script_path = None # currently active script path in editor/page stack
    ...
    def refresh_content_pages(self):
        ...
        for mod in sorted_mods_all:
            ...
            # Simplified button text to "Save Changes"
    ...
    def on_nav_row_changed(self, new_row):
        # Check if the previous script had unsaved modifications
        if self.current_script_path and self.current_script_path != "dashboard":
            if self.has_unsaved_changes(self.current_script_path):
                ...
                if response == QMessageBox.StandardButton.No:
                    # Block navigation change by reverting nav list index back
                    sorted_mods = self.sort_modules(self.modules)
        ...
        # Update current tracking path based on row selection
        if new_row == 0:
            self.current_script_path = "dashboard"
        else:
            sorted_mods = self.sort_modules(self.modules)
            if 0 <= new_row - 1 < len(sorted_mods):
                self.current_script_path = sorted_mods[new_row - 1]['path']
            else:
