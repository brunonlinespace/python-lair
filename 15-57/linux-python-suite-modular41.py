...
from PyQt6.QtGui import QDragEnterEvent, QDropEvent, QFont, QAction, QKeyEvent
...
class LinuxPythonLauncherEditor(QMainWindow):
    ...
    def update_nav_item_indicators(self):
        ...
        # Refresh dashboard banner dynamically if visible
    ...
    def refresh_content_pages(self):
        ...
        # Unsaved changes banner on dashboard
        ...
        if not self.modules:
            ...
            if self.dashboard_mode == "list":
                ...
                for mod in sorted_dashboard_mods:
                    card_frame = QFrame()
                    card_frame.setFixedSize(270, 140)
                    card_frame.setStyleSheet("""
                        QFrame {
                            background-color: #1e1e1e;
                            border: 1px solid #333338;
    ...
    def save_module_code(self, script_path, editor):
        try:
            ...
            # Update indicators and status without rebuilding or changing pages
            self.update_nav_item_indicators()
    ...
    def make_executable(self, script_path):
        try:
            ...
            self.update_status_label(f"Granted executable permission to {os.path.basename(script_path)}")
