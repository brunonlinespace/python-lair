...
import shutil
import subprocess
from PyQt6.QtCore import Qt, QUrl, QRect, QSize, QPoint, QTimer
from PyQt6.QtGui import QDragEnterEvent, QDropEvent, QFont
from PyQt6.QtWidgets import (
    QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
    QPushButton, QLabel, QStackedWidget, QFrame, QListWidget, QMessageBox, QTextEdit, QScrollArea, QLayout, QInputDialog, QLineEdit, QComboBox
)
...
class LinuxPythonLauncherEditor(QMainWindow):
    def __init__(self):
        ...
        self.sort_criterion = "title"  # options: title, created, modified
    ...
    def sort_modules(self, mods):
        if self.sort_criterion == "created":
    ...
    def refresh_content_pages(self):
        ...
        """)
        # Debounce the search input using QTimer so it doesn't rebuild UI on every keystroke and lose focus
        self.search_input.textChanged.connect(self.on_search_text_changed)
        controls_layout.addWidget(self.search_input, 1)

        sort_label = QLabel("Sort by:")
        sort_label.setStyleSheet("color: #ffffff; font-weight: bold;")
        controls_layout.addWidget(sort_label)

        self.sort_combo = QComboBox()
        self.sort_combo.addItems(["Title (A-Z)", "Last Created", "Last Modified"])
        if self.sort_criterion == "created":
        ...
        self.sort_combo.setStyleSheet("""
            ...
            QComboBox QAbstractItemView {
                background-color: #1e1e1e; color: #ffffff; selection-background-color: #007acc; selection-color: #ffffff;
            }
        """)
        ...
        # Filter and sort modules for the dashboard display
        filtered_mods = [
    ...
    def on_search_text_changed(self, text):
        ...
        # Use singleShot timer to debounce UI refresh so typing characters isn't interrupted by focus loss
        if hasattr(self, '_search_timer'):
            self._search_timer.stop()
        self._search_timer = QTimer()
        self._search_timer.setSingleShot(True)
        self._search_timer.timeout.connect(self.perform_search_refresh)
        self._search_timer.start(250)

    def perform_search_refresh(self):
        current_cursor_pos = self.search_input.cursorPosition()
        self.refresh_content_pages()
