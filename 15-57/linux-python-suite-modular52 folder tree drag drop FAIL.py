...
# Version: 52 (Fixed Drag & Drop Script Re-parenting Crash)
#=============================================================================
...
class AccessibleTreeWidget(QTreeWidget):
    ...
    def dropEvent(self, event: QDropEvent):
        # Capture the dragged item and its script path BEFORE calling super().dropEvent(),
        # because the internal tree model will delete/recreate nodes during the drop action.
        dragged_item = self.currentItem()
        script_path = dragged_item.data(0, Qt.ItemDataRole.UserRole) if dragged_item else None
        ...
        if script_path and script_path != "dashboard":
            # Determine target relative directory based on drop target
            target_rel_dir = ""
            if target_item:
                ...
                if target_data == "dashboard":
                    ...
                    # Dropped onto another script, use its folder parent
                    target_rel_dir = os.path.dirname(os.path.relpath(target_data, MODULES_DIR))
                else:
                    # Dropped onto a folder node or path string
                    # Reconstruct folder name path from tree hierarchy
                    path_parts = []
                    curr = target_item
            ...
            if script_path != new_path:
                try:
                    ...
                    # Notify main window to reload modules
                    window = self.window()
