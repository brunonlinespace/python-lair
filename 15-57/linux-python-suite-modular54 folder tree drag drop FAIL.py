...
# Version: 54 (Folder Management & Dedicated Folder View Page)
#=============================================================================
...
class AccessibleTreeWidget(QTreeWidget):
    ...
    def dropEvent(self, event: QDropEvent):
        ...
        if script_path and script_path != "dashboard" and not script_path.startswith("folder:"):
            target_rel_dir = ""
            if target_item:
                try:
                    ...
                    if target_data == "dashboard":
                        target_rel_dir = ""
                    elif target_data and target_data.startswith("folder:"):
                        target_rel_dir = target_data.replace("folder:", "")
                    elif target_data and os.path.isfile(target_data):
                        target_rel_dir = os.path.dirname(os.path.relpath(target_data, MODULES_DIR))
                    else:
                        path_parts = []
                        curr = target_item
                        while curr and curr != self.topLevelItem(0):
...
class LinuxPythonLauncherEditor(QMainWindow):
    def __init__(self):
        ...
        self.title_labels = {}
        self.chmod_buttons = {}
        self.folder_pages = {}       # rel_folder_path -> QWidget
    ...
    def refresh_content_pages(self):
        ...
        # Discover unique folders to generate folder pages
        unique_folders = sorted(list(set(mod['folder'] for mod in self.modules if mod['folder'] and mod['folder'] != ".")))
        for folder_rel in unique_folders:
            folder_page = QWidget()
            f_layout = QVBoxLayout(folder_page)
            f_layout.setContentsMargins(30, 30, 30, 30)

            abs_folder_path = os.path.abspath(os.path.join(MODULES_DIR, folder_rel))
            f_title = QLabel(f"<h2>📁 Folder: {folder_rel}</h2><font color='#858585'>{abs_folder_path}</font>")
            f_title.setWordWrap(True)
            f_layout.addWidget(f_title)

            f_btn_row = QHBoxLayout()
            f_btn_row.setSpacing(8)

            f_term_btn = QPushButton("Terminal")
            f_term_btn.setStyleSheet("padding: 8px 12px; font-weight: bold; background-color: #2d5a2d; color: #cfc; border: none; border-radius: 4px;")
            f_term_btn.setToolTip("Open this folder in an external terminal")
            f_term_btn.clicked.connect(lambda checked, fpath=abs_folder_path: self.launch_folder_in_terminal(fpath))

            f_rename_btn = QPushButton("Rename")
            f_rename_btn.setStyleSheet("padding: 8px 12px; background-color: #333337; color: #dcdcdc; border: 1px solid #555; border-radius: 4px;")
            f_rename_btn.clicked.connect(lambda checked, frel=folder_rel: self.rename_folder(frel))

            f_del_btn = QPushButton("Remove")
            f_del_btn.setStyleSheet("padding: 8px 12px; background-color: #511; color: #f88; border: none; border-radius: 4px;")
            f_del_btn.clicked.connect(lambda checked, frel=folder_rel: self.remove_folder(frel))

            f_btn_row.addWidget(f_term_btn)
            f_btn_row.addWidget(f_rename_btn)
            f_btn_row.addWidget(f_del_btn)
            f_btn_row.addStretch()
            f_layout.addLayout(f_btn_row)

            f_desc = QLabel(f"<b>Scripts contained inside {folder_rel}:</b>")
            f_desc.setStyleSheet("margin-top: 15px; font-size: 11pt;")
            f_layout.addWidget(f_desc)

            folder_mods = [m for m in self.modules if m['folder'] == folder_rel]
            f_list_layout = QVBoxLayout()
            f_list_layout.setSpacing(10)

            for mod in folder_mods:
                card = QFrame()
                card.setStyleSheet("background-color: #1e1e1e; border: 1px solid #333338; border-radius: 6px;")
                cl = QHBoxLayout(card)
                cl.setContentsMargins(12, 10, 12, 10)
                
                info = QLabel(f"<b>{mod['name']}</b><br><font color='#858585' size='9pt'>{mod['path']}</font>")
                info.setStyleSheet("border: none; background: transparent;")
                
                b_layout = QHBoxLayout()
                b_layout.setSpacing(6)
                
                l_btn = QPushButton("▶ Launch")
                l_btn.setStyleSheet("background-color: #007acc; color: white; border: none; padding: 4px 8px; border-radius: 3px; font-weight: bold;")
                l_btn.clicked.connect(lambda checked, p=mod['path']: self.launch_external_persistent(p))

                e_btn = QPushButton("Edit")
                e_btn.setStyleSheet("background-color: #333337; color: #dcdcdc; border: 1px solid #555; border-radius: 3px; padding: 4px 8px;")
                e_btn.clicked.connect(lambda checked, p=mod['path']: self.jump_to_module_page(p))

                b_layout.addWidget(l_btn)
                b_layout.addWidget(e_btn)

                cl.addWidget(info, 1)
                cl.addLayout(b_layout)
                f_list_layout.addWidget(card)

            f_scroll = QScrollArea()
            f_scroll.setWidgetResizable(True)
            f_scroll.setStyleSheet("QScrollArea { border: none; background: transparent; }")
            container_w = QWidget()
            container_w.setLayout(f_list_layout)
            f_scroll.setWidget(container_w)
            f_layout.addWidget(f_scroll, 1)

            self.folder_pages[folder_rel] = folder_page
    ...
    def jump_to_module_page(self, script_path):
        ...
        if script_path.startswith("folder:"):
            folder_rel = script_path.replace("folder:", "")
            unique_folders = sorted(list(set(mod['folder'] for mod in self.modules if mod['folder'] and mod['folder'] != ".")))
            if folder_rel in unique_folders:
                # Dashboard is index 0, folder pages follow before script pages
                folder_idx = unique_folders.index(folder_rel) + 1
                self.switch_view(folder_idx)
            return

        unique_folders = sorted(list(set(mod['folder'] for mod in self.modules if mod['folder'] and mod['folder'] != ".")))
        offset = 1 + len(unique_folders)

        sorted_mods = self.sort_modules(self.modules)
        for idx, mod in enumerate(sorted_mods, start=offset):
    ...
    def on_nav_item_clicked(self, item, column):
        ...
        if target_path == "dashboard":
            ...
            folder_rel = target_path.replace("folder:", "")
            unique_folders = sorted(list(set(mod['folder'] for mod in self.modules if mod['folder'] and mod['folder'] != ".")))
            if folder_rel in unique_folders:
                folder_idx = unique_folders.index(folder_rel) + 1
                self.switch_view(folder_idx)
    ...
    def rename_folder(self, folder_rel):
        old_abs_path = os.path.abspath(os.path.join(MODULES_DIR, folder_rel))
        parent_dir = os.path.dirname(folder_rel)
        current_folder_name = os.path.basename(folder_rel)
        
        new_name, ok = QInputDialog.getText(self, "Rename Folder", "Enter new folder name:", text=current_folder_name)
        if ok and new_name:
            new_name = new_name.strip()
            if not new_name or new_name == current_folder_name:
                return
            
            new_rel_folder = os.path.join(parent_dir, new_name) if parent_dir and parent_dir != "." else new_name
            new_abs_path = os.path.abspath(os.path.join(MODULES_DIR, new_rel_folder))
            ...
            try:
                ...
                self.refresh_modules_from_disk()
                self.jump_to_module_page(f"folder:{new_rel_folder}")
                self.update_status_label(f"Renamed folder to {new_name}")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Could not rename folder: {e}")

    def remove_folder(self, folder_rel):
        abs_folder_path = os.path.abspath(os.path.join(MODULES_DIR, folder_rel))
        confirm = QMessageBox.question(
            self, "Remove Folder", 
            f"Are you sure you want to remove folder '{folder_rel}' and all scripts inside it?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
        if confirm == QMessageBox.StandardButton.Yes:
            try:
                for mod in self.modules:
                    if mod['folder'] == folder_rel:
                        p = mod['path']
                ...
                self.update_status_label(f"Removed folder {folder_rel}")
            except Exception as e:
    ...
    def launch_folder_in_terminal(self, folder_abs_path):
        if not os.path.exists(folder_abs_path):
            QMessageBox.warning(self, "Missing Folder", f"The folder path does not exist.")
            return

        terminals = [
            ("gnome-terminal", ["gnome-terminal", "--working-directory", folder_abs_path]),
            ("konsole", ["konsole", "--workdir", folder_abs_path]),
            ("xfce4-terminal", ["xfce4-terminal", "--working-directory", folder_abs_path]),
            ("tilix", ["tilix", "-w", folder_abs_path]),
            ("terminator", ["terminator", "--working-directory", folder_abs_path]),
            ("alacritty", ["alacritty", "--working-directory", folder_abs_path]),
            ("kitty", ["kitty", "--directory", folder_abs_path]),
            ("foot", ["foot", "-D", folder_abs_path]),
            ("xterm", ["xterm", "-e", f"cd '{folder_abs_path}' && exec $SHELL"])
        ]
        ...
        for term_name, cmd in terminals:
            if shutil.which(term_name):
                try:
                    subprocess.Popen(cmd)
                    self.update_status_label(f"Opened folder in {term_name}.")
                    launched = True
                    break
                except Exception as e:
                    print(f"Failed to launch folder via {term_name}: {e}")

        if not launched:
            QMessageBox.critical(self, "Terminal Error", "No supported external Linux terminal emulator found.")
