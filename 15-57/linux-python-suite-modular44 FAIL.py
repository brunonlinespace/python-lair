...
from PyQt6.QtGui import QDragEnterEvent, QDropEvent, QFont, QAction, QKeyEvent, QPainter, QColor, QTextFormat
from PyQt6.QtWidgets import (
    QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
    QPushButton, QLabel, QStackedWidget, QFrame, QListWidget, QMessageBox, QTextEdit, QPlainTextEdit, QScrollArea, QLayout, QInputDialog, QLineEdit, QComboBox, QSplitter
)
...
class CodeEditor(QPlainTextEdit):
    """A customized QPlainTextEdit that includes a line number sidebar."""
    ...
    def line_number_area_paint_event(self, event):
        ...
        bottom = top + int(self.blockBoundingRectangle(block).height())
...
class AccessibleListWidget(QListWidget):
    """Custom QListWidget that triggers navigation on arrow key presses."""
...
class LinuxPythonLauncherEditor(QMainWindow):
    ...
    def setup_sidebar(self):
        ...
        self.nav_list = AccessibleListWidget()
        ...
        if not self.modules:
            ...
            if self.dashboard_mode == "list":
                ...
                for mod in sorted_dashboard_mods:
                    ...
                    info_lbl = QLabel(f"<span style='color: #ffffff; font-size: 11pt;'><b>{mod['name']}</b></span>{exec_badge}<br><span style='color: #858585; font-size: 9pt;'>{mod['filename']}</span>")
                ...
                for mod in sorted_dashboard_mods:
                    ...
                    meta_lbl = QLabel(f"<font color='#858585' size='2'>Created: {created_time_str}<br>Modified: {mod_time_str}<br>Size: {size_str}</font>")
    ...
    def on_nav_item_clicked(self, item):
        ...
        new_row = self.nav_list.row(item)
    ...
    def closeEvent(self, event):
        ...
        if unsaved_paths:
            ...
            save_btn = msg_box.addButton("Save All & Exit", QMessageBox.ButtonRole.AcceptRole)
