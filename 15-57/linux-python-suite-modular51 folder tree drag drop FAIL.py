...
# Version: 51 (Added Drag & Drop Folder Reorganization for Scripts)
#=============================================================================
...
class AccessibleTreeWidget(QTreeWidget):
    ...
    def dropEvent(self, event: QDropEvent):
        # Identify dragged item before standard drop executes
        dragged_item = self.currentItem()
        target_item = self.itemAt(event.position().toPoint())
        
        super().dropEvent(event)
        
        if dragged_item and target_item:
            script_path = dragged_item.data(0, Qt.ItemDataRole.UserRole)
            if not script_path or script_path == "dashboard":
                return
