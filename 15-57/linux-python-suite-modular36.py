...
from PyQt6.QtGui import QDragEnterEvent, QDropEvent, QFont, QAction
from PyQt6.QtWidgets import (
    QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
    QPushButton, QLabel, QStackedWidget, QFrame, QListWidget, QMessageBox, QTextEdit, QScrollArea, QLayout, QInputDialog, QLineEdit, QComboBox, QSplitter
)
...
class LinuxPythonLauncherEditor(QMainWindow):
    def __init__(self):
        ...
        # Track editors and original contents to check for unsaved modifications
        self.editors = {}          # path -> QTextEdit
    ...
    def setup_sidebar(self):
        ...
        self.status_lbl.setStyleSheet("color: #9cdcfe; font-size: 9pt; margin-top: 5px;")
    ...
    def refresh_content_pages(self):
        ...
        if not self.modules:
            ...
            if self.dashboard_mode == "list":
                ...
                for mod in sorted_dashboard_mods:
                    card_frame = QFrame()
                    card_frame.setFixedSize(270, 140)  # Decreased card size
                    card_frame.setStyleSheet("""
                        QFrame {
                            background-color: #1e1e1e;
                            border: 1px solid #333338;
                    ...
                    mod_time_str = datetime.fromtimestamp(mod['modified']).strftime('%Y-%m-%d %H:%M') if mod['modified'] else "Unknown"
                    size_str = self.format_size(mod['size'])

                    meta_lbl = QLabel(f"<font color='#858585' size='2'>Modified: {mod_time_str}<br>Size: {size_str}</font>")
        ...
        for mod in sorted_mods_all:
            ...
            # Connect text change to update indicator
    ...
    def on_sort_changed(self, index):
        ...
        current_row = self.nav_list.currentRow()
        if current_row > 0:
            sorted_old = self.sort_modules(self.modules)
            if current_row - 1 < len(sorted_old):
                current_path = sorted_old[current_row - 1]['path']

        self.populate_nav_list()
        self.refresh_content_pages()

        if current_path:
            sorted_new = self.sort_modules(self.modules)
            for i, m in enumerate(sorted_new):
                if m['path'] == current_path:
                    self.nav_list.setCurrentRow(i + 1)
                    break
        else:
            self.nav_list.setCurrentRow(0)
    ...
    def on_nav_row_changed(self, new_row):
        if self.is_navigating:
            return

        # Determine the target path for the newly selected row
        if new_row == 0:
            target_path = "dashboard"
        else:
            sorted_mods = self.sort_modules(self.modules)
            if 0 <= new_row - 1 < len(sorted_mods):
                target_path = sorted_mods[new_row - 1]['path']
            else:
                target_path = None

        # Check if the currently active script has unsaved modifications
        if self.current_script_path and self.current_script_path != "dashboard" and self.current_script_path != target_path:
            if self.has_unsaved_changes(self.current_script_path):
                ...
                if response == QMessageBox.StandardButton.No:
                    # User clicked "No": Keep the sidebar selector on the original row without changing views
                    self.is_navigating = True
                    self.nav_list.blockSignals(True)
                    if self.current_script_path == "dashboard":
                        old_row = 0
                    else:
                        old_row = 0
                        sorted_mods = self.sort_modules(self.modules)
                        for idx, mod in enumerate(sorted_mods, start=1):
                            if mod['path'] == self.current_script_path:
                                old_row = idx
                                break
                    self.nav_list.setCurrentRow(old_row)
                    self.nav_list.blockSignals(False)
                    self.is_navigating = False
                    return

        # Update current tracking path and switch view
        self.current_script_path = target_path
        self.switch_view(new_row)
