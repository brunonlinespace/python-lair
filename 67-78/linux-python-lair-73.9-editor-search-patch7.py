...
# Version: 73 (Editor Search Function Patched)
# ==============================================================================
...
from PyQt6.QtGui import QDragEnterEvent, QDropEvent, QFont, QAction, QKeyEvent, QPainter, QColor, QTextDocument
...
class LinuxPythonLauncherEditor(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Python Lair (v74) - Simple Launcher & Editor")
        ...
        try:
            if os.path.exists(CONFIG_FILE):
                with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    dash_mode = data.get("dashboard_mode", "list")
                    sort_crit = data.get("sort_criterion", "modified")
        except Exception:
            pass
        return dash_mode, sort_crit
    ...
    def save_config_state(self):
        try:
            ...
            data["dashboard_mode"] = self.dashboard_mode
            data["sort_criterion"] = self.sort_criterion
    ...
    def setup_sidebar(self):
        ...
        self.nav_tree.itemCollapsed.connect(self.on_tree_item_collapsed)
        self.nav_tree.itemExpanded.connect(self.on_tree_item_expanded)
    ...
    def refresh_content_pages(self):
        ...
        for mod in sorted_mods_all:
            ...
            btn_row.addWidget(ext_edit_btn)
            ...
            # Simple text search box on the bottom left side before cancel/save changes buttons
            editor_search_input = QLineEdit()
            editor_search_input.setPlaceholderText("Find text in script...")
            editor_search_input.setFixedWidth(220)
            editor_search_input.setStyleSheet("padding: 6px; background-color: #1e1e1e; color: #dcdcdc; border: 1px solid #3f3f46; border-radius: 4px;")
            
            editor_find_btn = QPushButton("🔍 Find")
            editor_find_btn.setFixedWidth(80)
            editor_find_btn.setStyleSheet("padding: 6px; background-color: #333337; color: #ccc; border: 1px solid #555; border-radius: 4px;")
            editor_find_btn.clicked.connect(lambda checked, ed=text_editor, inp=editor_search_input: self.find_text_in_editor(ed, inp))
            
            # (1) Allow finding text by pressing Enter on the keyboard inside the search box
            editor_search_input.returnPressed.connect(lambda ed=text_editor, inp=editor_search_input: self.find_text_in_editor(ed, inp))

            editor_btn_layout.addWidget(editor_search_input)
            editor_btn_layout.addWidget(editor_find_btn)
    ...
    def find_text_in_editor(self, editor, search_input):
        ...
        # Search document starting from current cursor position
        found_cursor = document.find(query, cursor, QTextDocument.FindFlag(0))
        
        # (2) Loop back to the original search start (beginning of doc) when reaching the end
        if found_cursor.isNull():
            zero_cursor = editor.textCursor()
            zero_cursor.movePosition(QTextDocument.MoveOperation.Start)
            found_cursor = document.find(query, zero_cursor, QTextDocument.FindFlag(0))
            if not found_cursor.isNull():
                editor.setTextCursor(found_cursor)
                self.update_status_label(f"Reached end. Wrapped around and found: '{query}'")
                return
        
        if not found_cursor.isNull():
            editor.setTextCursor(found_cursor)
            self.update_status_label(f"Found text: '{query}'")
        else:
            self.update_status_label(f"Text not found: '{query}'")
            QMessageBox.information(self, "Find", f"Could not find '{query}' in the script.")
    ...
    def on_sort_changed(self, index):
        ...
        # Save the updated sort criterion preference to JSON config
        self.save_config_state()
    ...
    def on_nav_item_clicked(self, item, column):
        ...
        if not target_path: # Folder header clicked
            return
    ...
    def open_in_external_editor(self, script_path):
        ...
        editors = [
            ("code", ["code", abs_path]),
            ("codium", ["codium", abs_path]),
            ("gedit", ["gedit", abs_path]),
            ("kate", ["kate", abs_path]),
            ("mousepad", ["mousepad", abs_path]),
            ("geany", ["geany", abs_path]),
            ("nano", ["gnome-terminal", "--", "nano", abs_path])
        ]

        opened = False
        for ed_name, cmd in editors:
            bin_to_check = cmd[0] if cmd[0] != "gnome-terminal" else "nano"
            if shutil.which(bin_to_check):
                try:
                    subprocess.Popen(cmd)
                    self.update_status_label(f"Opened {os.path.basename(script_path)} in {ed_name}.")
                    opened = True
                    break
                except Exception as e:
                    print(f"Failed to launch via {ed_name}: {e}")

        if not opened:
            try:
                subprocess.Popen(["xdg-open", abs_path])
                self.update_status_label(f"Opened {os.path.basename(script_path)} using system default handler.")
                opened = True
            except Exception as e:
