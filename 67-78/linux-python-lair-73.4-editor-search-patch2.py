...
class LinuxPythonLauncherEditor(QMainWindow):
    ...
    def find_text_in_editor(self, editor, search_input):
        ...
        # Search document starting from current cursor position, wrap around if needed
        document = editor.document()
        cursor = editor.textCursor()

        found_crypto = document.find(query, cursor)
        if found_crypto.isNull():
            ...
            found_crypto = document.find(query, zero_cursor)

        if not found_crypto.isNull():
            editor.setTextCursor(found_crypto)
