...
class LinuxPythonLauncherEditor(QMainWindow):
    ...
    def find_text_in_editor(self, editor, search_input):
        ...
        # Perform forward search from current cursor position
        found_cursor = document.find(query, cursor, QTextDocument.FindFlags())

        # If not found, wrap around to the absolute beginning of the document
        if found_cursor.isNull():
            zero_cursor = QTextCursor(document)
            zero_cursor.movePosition(QTextCursor.MoveOperation.Start)
            found_cursor = document.find(query, zero_cursor, QTextDocument.FindFlags())
