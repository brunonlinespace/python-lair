...
class LinuxPythonLauncherEditor(QMainWindow):
    ...
    def find_text_in_editor(self, editor, search_input):
        ...
        # Try to find the next occurrence from the current cursor position
        found_cursor = document.find(query, cursor)

        # If nothing found from current position (or we reached the end), wrap around to the top
        ...
        # If still nothing found anywhere in the document
        if found_cursor.isNull():
