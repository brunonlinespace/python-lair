...
class LinuxPythonLauncherEditor(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Python Lair (v73) - Simple Launcher & Editor")
    ...
    def refresh_content_pages(self):
        ...
        for mod in sorted_mods_all:
            ...
            editor_search_input.returnPressed.connect(lambda: editor_find_btn.click())
    ...
    def find_text_in_editor(self, editor, search_input):
        ...
        found_cursor = document.find(query, cursor)

        # If not found or we are stuck on the current selection, wrap around to start
        if found_cursor.isNull() or (found_cursor.position() == cursor.position() and found_cursor.anchor() == cursor.anchor()):
            zero_cursor = editor.textCursor()
            zero_cursor.movePosition(QTextDocument.MoveOperation.Start)
            found_cursor = document.find(query, zero_cursor)

        # Final safety check: if still null, display message safely
        if found_cursor.isNull() or not found_cursor.isValid():
