...
# Version: 68 (Accidental Collapse)
#=============================================================================
...
class LinuxPythonLauncherEditor(QMainWindow):
    ...
    def setup_sidebar(self):
        ...
        self.populate_nav_tree(is_initial=True)
    ...
    def populate_nav_tree(self, is_initial=False):
        # 1. Capture the expansion state of existing folder nodes before clearing
        expanded_folders = set()
        def save_expansion_state(parent_item):
            for i in range(parent_item.childCount()):
                child = parent_item.child(i)
                folder_path = child.data(0, Qt.ItemDataRole.UserRole + 1)
                if folder_path:
                    if child.isExpanded():
        ...
        for i in range(self.nav_tree.topLevelItemCount()):
            save_expansion_state(self.nav_tree.topLevelItem(i))
        ...
        # 2. Restore expansion state, or expand all only if it's the initial load/refresh
        if is_initial:
            self.nav_tree.expandAll()
        else:
            def restore_expansion_state(parent_item):
                for i in range(parent_item.childCount()):
                    child = parent_item.child(i)
                    folder_path = child.data(0, Qt.ItemDataRole.UserRole + 1)
                    if folder_path:
                        ...
                        restore_expansion_state(child)

            for i in range(self.nav_tree.topLevelItemCount()):
                restore_expansion_state(self.nav_tree.topLevelItem(i))

    def update_nav_item_indicators(self):
        self.populate_nav_tree(is_initial=False)
        self.jump_to_module_page(self.current_script_path)
    ...
    def refresh_content_pages(self):
        ...
        title_label = QLabel("<h1>Python Lair - Simple Launcher & Editor (v68)</h1>")
