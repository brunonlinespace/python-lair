...
# Version: 81 (Card Borders, Specific Button Colors, & Editor Crash Fix)
# ==============================================================================
...
class LinuxPythonLauncherEditor(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Python Lair (v81) - Simple Launcher & Editor")
    ...
    def setup_sidebar(self):
        ...
        info_drop_lbl = QLabel("Tip: Drag & drop .py scripts onto the Dashboard to add them!")
        ...
        if self.current_script_path in self.title_labels:
            ...
            if mod_info:
                ...
                if self.has_unsaved_changes(self.current_script_path):
                    title_lbl.setText(f"<h2>{mod_info['name']} [Unsaved Modifications]</h2><font color='#858585'>{mod_info['path']}</font>")
                else:
        ...
        if not self.modules:
            ...
            if self.dashboard_mode == "list":
                ...
                for mod in sorted_dashboard_mods:
                    ...
                    launch_btn.setStyleSheet("background-color: #2b78e4; color: white; border-radius: 4px; padding: 5px 10px;")
                ...
                for mod in sorted_dashboard_mods:
                    ...
                    launch_btn = QPushButton("Launch")
                    launch_btn.setStyleSheet("background-color: #2b78e4; color: white; border-radius: 3px; padding: 3px 6px;")
                    launch_btn.clicked.connect(lambda checked, path=mod['path']: self.launch_external_persistent(path))
                    ...
                    del_btn.setStyleSheet("background-color: #d93838; color: white; border-radius: 3px; padding: 3px 6px;")
                    del_btn.clicked.connect(lambda checked, path=mod['path']: self.remove_module(path))
    ...
    def clear_search_filter(self):
        ...
        if hasattr(self, 'search_input'):
            self.search_input.setFocus()
        self.update_status_label("Search cleared.")
