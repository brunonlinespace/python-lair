...
from PyQt6.QtCore import Qt, QUrl, QRect, QSize, QPoint, QSignalBlocker
...
class AccessibleTreeWidget(QTreeWidget):
    """Custom QTreeWidget that safely triggers navigation changes on key navigation."""
...
class LinuxPythonLauncherEditor(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Python Lair - Simple Launcher & Editor")
        ...
        self.sort_criterion = "modified"  # Default sorting is "last modified"
        ...
        self.current_script_path = "dashboard" # currently active script path or folder token in stack
    ...
    def setup_sidebar(self):
        ...
        info_drop_lbl = QLabel("<font color='#9cdcfe'>💡 Tip: Drag & drop .py scripts onto dashboards to add them!</font>")
    ...
    def refresh_modules_from_disk(self):
        ...
        self.jump_to_page(self.current_script_path)
        ...
        for mod in self.modules:
            ...
            if folder and folder != ".":
                ...
                for i in range(1, len(parts) + 1):
                    subfolders.add(os.path.join(*parts[:i]))
    ...
    def populate_nav_tree(self):
        ...
        for sf in subfolders:
            ...
            for part in parts:
                ...
                if current_accumulated not in folder_nodes:
                    ...
                    folder_node.setData(0, Qt.ItemDataRole.UserRole, f"folder:{current_accumulated}")
        ...
        for mod in sorted_mods:
            ...
            if not folder_path or folder_path == ".":
                ...
                parent_widget = folder_nodes[folder_path]
    ...
    def refresh_content_pages(self):
        ...
        # Map token -> stack index
        self.page_mapping = {}

        # 1. Main Dashboard Page (Index 0)
        main_dash_widget = self.create_dashboard_widget(
            title_text="<h1>Python Lair - Simple Launcher & Editor</h1>",
            desc_text="Welcome! <b>Drag and drop any Python (.py) script file directly onto this window</b> or edit files externally in the <b>my library/</b> folder, then click <b>Refresh Modules</b>.",
            filter_folder=None
        )
        self.stack.addWidget(main_dash_widget)
        self.page_mapping["dashboard"] = 0

        # 2. Subfolder Dashboard Pages
        for sf in self.get_all_subfolders():
            abs_sf_path = os.path.join(MODULES_DIR, sf)
            sf_widget = self.create_dashboard_widget(
                title_text=f"<h1>📁 Folder: {sf}</h1>",
                desc_text=f"Managing python scripts inside <b>{abs_sf_path}</b>.",
                filter_folder=sf,
                folder_token=sf
            )
            idx = self.stack.addWidget(sf_widget)
            self.page_mapping[f"folder:{sf}"] = idx

        # 3. Individual Script Editor Pages
        ...
        for mod in sorted_mods_all:
            ...
            self.page_mapping[mod['path']] = idx

    def create_dashboard_widget(self, title_text, desc_text, filter_folder=None, folder_token=None):
        ...
        if folder_token:
            ...
            folder_term_btn = QPushButton("Terminal")
            folder_term_btn.setToolTip("Launch an external interactive terminal emulator in this folder")
            folder_term_btn.setStyleSheet("background-color: #2d5a2d; color: #cfc; border: none; padding: 6px 12px; border-radius: 4px; font-weight: bold;")
            folder_term_btn.clicked.connect(lambda checked, ft=folder_token: self.launch_folder_in_terminal(ft))

            folder_ren_btn = QPushButton("Rename Folder")
            folder_ren_btn.setStyleSheet("background-color: #333337; color: #dcdcdc; border: 1px solid #555; border-radius: 4px; padding: 6px 12px;")
            folder_ren_btn.clicked.connect(lambda checked, ft=folder_token: self.rename_folder(ft))

            folder_del_btn = QPushButton("Remove Folder")
            folder_del_btn.setStyleSheet("background-color: #511; color: #f88; border: none; padding: 6px 12px; border-radius: 4px;")
            folder_del_btn.clicked.connect(lambda checked, ft=folder_token: self.remove_folder(ft))

            folder_btn_row.addWidget(folder_term_btn)
            folder_btn_row.addWidget(folder_ren_btn)
            folder_btn_row.addWidget(folder_del_btn)
            folder_btn_row.addStretch()
            dash_layout.addLayout(folder_btn_row)
        ...
        base_filtered = self.modules
        if filter_folder:
            base_filtered = [
                m for m in self.modules 
                if m['folder'] == filter_folder or m['folder'].startswith(filter_folder + os.sep)
            ]

        filtered_mods = [
            m for m in base_filtered 
        ...
        if not base_filtered:
            no_mod_lbl = QLabel("<i>No python scripts found in this folder.</i>")
            ...
            if self.dashboard_mode == "list":
                ...
                for mod in sorted_dashboard_mods:
                    ...
                    edit_btn.clicked.connect(lambda checked, p=mod['path']: self.jump_to_page(p))
                    ...
                    links_layout.addWidget(card_frame)

                dash_layout.addLayout(links_layout)

            else:
                flow_layout = FlowLayout(margin=0, spacing=12)
                ...
                dash_layout.addLayout(flow_layout)
    ...
    def jump_to_page(self, token):
        self.current_script_path = token
        if token in self.page_mapping:
            self.switch_view(self.page_mapping[token])

        def find_and_select_item(parent_item):
            for i in range(parent_item.childCount()):
                child = parent_item.child(i)
                if child.data(0, Qt.ItemDataRole.UserRole) == token:
                    # --- FIX: Set guard flag before changing current item ---
                    self.is_navigating = True
        ...
        for i in range(self.nav_tree.topLevelItemCount()):
            ...
            if top_item.data(0, Qt.ItemDataRole.UserRole) == token:
                # --- FIX: Set guard flag before changing current item ---
                self.is_navigating = True
                ...
                # ---------------------------------------------------------
                break
            if find_and_script := find_and_select_item(top_item):
                break
    ...
    def on_nav_item_clicked(self, item, column=0):
        if self.is_navigating or not item:
            return

        target_token = item.data(0, Qt.ItemDataRole.UserRole)
        if not target_token:
            return

        if target_token == self.current_script_path:
            return

        # Check for unsaved changes on the current active script
        if self.current_script_path and self.current_script_path != "dashboard" and not self.current_script_path.startswith("folder:"):
            if self.has_unsaved_changes(self.current_script_path):
                ...
                if response == QMessageBox.StandardButton.No:
                    # User chose NO: Keep the editor on the current script and
                    # forcefully lock the sidebar selection back to the current script item.
                    self.is_navigating = True
                    blocker = QSignalBlocker(self.nav_tree)

                    # Find the tree item matching the current active script path and select it
                    def select_current_token_item(parent_item):
                        for i in range(parent_item.childCount()):
                            child = parent_item.child(i)
                            if child.data(0, Qt.ItemDataRole.UserRole) == self.current_script_path:
                                self.nav_tree.setCurrentItem(child)
                                return True
                            if select_current_token_item(child):
                                return True
                        return False

                    for i in range(self.nav_tree.topLevelItemCount()):
                        top_item = self.nav_tree.topLevelItem(i)
                        if top_item.data(0, Qt.ItemDataRole.UserRole) == self.current_script_path:
                            self.nav_tree.setCurrentItem(top_item)
                            break
                        if select_current_token_item(top_item):
                            break
        ...
        self.current_script_path = target_token
        if target_token in self.page_mapping:
            self.switch_view(self.page_mapping[target_token])
    ...
    def rename_folder(self, folder_token):
        abs_folder_path = os.path.join(MODULES_DIR, folder_token)
        parent_dir = os.path.dirname(abs_folder_path)
        current_dirname = os.path.basename(abs_folder_path)

        new_name, ok = QInputDialog.getText(self, "Rename Folder", "Enter new folder name:", text=current_dirname)
        if ok and new_name:
            new_name = new_name.strip()
            if not new_name or new_name == current_dirname:
                return

            new_abs_path = os.path.join(parent_dir, new_name)
            if os.path.exists(new_abs_path):
                QMessageBox.warning(self, "Error", f"A folder named '{new_name}' already exists.")
                return

            try:
                os.rename(abs_folder_path, new_abs_path)
                rel_parent = os.path.relpath(parent_dir, MODULES_DIR)
                new_token = new_name if rel_parent == "." else os.path.join(rel_parent, new_name)
                
                self.refresh_modules_from_disk()
                self.jump_to_page(f"folder:{new_token}")
                self.update_status_label(f"Renamed folder to {new_name}")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Could not rename folder: {e}")

    def remove_folder(self, folder_token):
        abs_folder_path = os.path.join(MODULES_DIR, folder_token)
        confirm = QMessageBox.question(
            self, "Remove Folder", 
            f"Are you sure you want to remove folder '{folder_token}' and all scripts inside it from the workspace?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
        if confirm == QMessageBox.StandardButton.Yes:
            try:
                # Terminate any running processes for scripts inside this folder
                for path in list(self.processes.keys()):
                    if path.startswith(abs_folder_path + os.sep) or path == abs_folder_path:
                        if self.processes[path].poll() is None:
                            self.processes[path].terminate()
                ...
                self.refresh_modules_from_disk()
                self.jump_to_page("dashboard")
                self.update_status_label(f"Removed folder {folder_token}")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Could not remove folder: {e}")
    ...
    def dropEvent(self, event: QDropEvent):
        ...
        # If currently viewing a subfolder dashboard, drop files directly into that subfolder
        if self.current_script_path and self.current_script_path.startswith("folder:"):
            target_dir = os.path.join(MODULES_DIR, self.current_script_path[len("folder:"):])
    ...
    def launch_folder_in_terminal(self, folder_token):
        abs_folder_path = os.path.abspath(os.path.join(MODULES_DIR, folder_token))
        if not os.path.exists(abs_folder_path):
            QMessageBox.warning(self, "Missing Folder", f"The folder path {abs_folder_path} does not exist.")
            return

        terminals = [
            ("gnome-terminal", ["gnome-terminal", "--working-directory", abs_folder_path]),
            ("konsole", ["konsole", "--workdir", abs_folder_path]),
            ("xfce4-terminal", ["xfce4-terminal", "--working-directory", abs_folder_path]),
            ("tilix", ["tilix", "--working-directory", abs_folder_path]),
            ("terminator", ["terminator", "--working-directory", abs_folder_path]),
            ("alacritty", ["alacritty", "--working-directory", abs_folder_path]),
            ("kitty", ["kitty", "--directory", abs_folder_path]),
            ("foot", ["foot", "-D", abs_folder_path]),
            ("xterm", ["xterm", "-e", f"cd {abs_folder_path} && bash"])
        ]
        ...
        for term_name, cmd in terminals:
            if shutil.which(term_name):
                try:
                    subprocess.Popen(cmd)
                    self.update_status_label(f"Opened folder {folder_token} in {term_name}.")
                    launched = True
                    break
                except Exception as e:
                    print(f"Failed to launch via {term_name}: {e}")
