...
# -*- coding: utf-8 -*-
#
# Dynamic Linux Productivity Suite Launcher with Fixed Process Verification
...
class DynamicProductivitySuite(QMainWindow):
    ...
    def launch_external_persistent(self, script_path):
        if os.path.exists(script_path):
            try:
                # Check if an existing process for this script is already running and active
                if script_path in self.processes and self.processes[script_path].poll() is None:
                    self.status_lbl.setText(f"Status: {os.path.basename(script_path)} is already active.")
                else:
                    # Spawn process and immediately verify it initialized properly
                    proc = subprocess.Popen([sys.executable, script_path])
                    
                    # Brief check to see if process crashed instantly upon startup
