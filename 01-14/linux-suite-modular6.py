...
# -*- coding: utf-8 -*-
#
# Dynamic Linux Productivity Suite Launcher with Dashboard Actions
...
from PyQt6.QtWidgets import (
    ...
    QPushButton, QLabel, QStackedWidget, QFrame, QListWidget, QMessageBox, QTextEdit, QGridLayout
)
...
class DynamicProductivitySuite(QMainWindow):
    ...
    def refresh_content_pages(self):
        ...
        # Page 0: Dynamic Dashboard Home with Card/Row controls
        dash_widget = QWidget()
        ...
        if not self.modules:
            ...
            for index, mod in enumerate(self.modules, start=1):
                ...
                info_lbl = QLabel(f"<b>{mod['name']}</b><br><font color='#858585'>{mod['filename']}</font>")
                info_lbl.setStyleSheet("border: none; background: transparent;")
                
                # Action Buttons for Dashboard Item
                btn_layout = QHBoxLayout()
                btn_layout.setSpacing(6)

                launch_btn = QPushButton("▶ Launch")
                launch_btn.setStyleSheet("background-color: #007acc; color: white; border: none; padding: 6px 12px; border-radius: 4px; font-weight: bold;")
                launch_btn.clicked.connect(lambda checked, path=mod['path']: self.launch_external_persistent(path))

                edit_btn = QPushButton("✏ Edit")
                edit_btn.setStyleSheet("background-color: #333337; color: #dcdcdc; border: 1px solid #555; border-radius: 4px; padding: 6px 12px;")
                edit_btn.clicked.connect(lambda checked, idx=index: self.nav_list.setCurrentRow(idx))

                del_btn = QPushButton("🗑 Remove")
                del_btn.setStyleSheet("background-color: #511; color: #f88; border: none; padding: 6px 12px; border-radius: 4px;")
