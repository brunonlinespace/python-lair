...
# -*- coding: utf-8 -*-
#
# Dynamic Linux Productivity Suite Launcher with Responsive Grid Layout & State Memory
...
from PyQt6.QtWidgets import (
    ...
    QPushButton, QLabel, QStackedWidget, QFrame, QListWidget, QMessageBox, QTextEdit, QScrollArea, QLayout, QStyle
)
...
class FlowLayout(QLayout):
    """A custom layout that arranges widgets in a left-to-right, top-to-bottom flow, wrapping dynamically."""
    ...
    def doLayout(self, rect, test_only):
        ...
        for item in self.itemList:
            ...
            hspace = spacing + style.combinedMetric(QStyle.PixelMetric.PM_LayoutHorizontalSpacing, None, item.widget())
            vspace = spacing + style.combinedMetric(QStyle.PixelMetric.PM_LayoutVerticalSpacing, None, item.widget())
            
            next_x = x + item.sizeHint().width() + hspace
            if next_x - hspace > rect.right() and x > rect.x():
                x = rect.x()
...
class DynamicProductivitySuite(QMainWindow):
    ...
    def refresh_modules_from_disk(self):
        ...
        if diff > 0:
            msg = f"Refreshed: Found {diff} new module(s)."
        elif diff < 0:
            msg = f"Refreshed: {abs(diff)} module(s) removed."
        else:
    ...
    def refresh_content_pages(self):
        ...
        title_label = QLabel("<h1>Welcome to the Linux Productivity Suite</h1>")
        ...
        if not self.modules:
            no_mod_lbl = QLabel("<i>No modules found. Add scripts to the modules folder or drop them here!</i>")
            ...
            if self.dashboard_mode == "list":
                ...
                for index, mod in enumerate(self.modules, start=1):
                    ...
                    launch_btn.setStyleSheet("background-color: #007acc; color: white; border: none; padding: 6px 14px; border-radius: 4px; font-weight: bold;")
                    launch_btn.clicked.connect(lambda checked, path=mod['path']: self.launch_external_persistent(path))

                    edit_btn = QPushButton("✏ Edit")
                    edit_btn.setStyleSheet("background-color: #333337; color: #dcdcdc; border: 1px solid #555; border-radius: 4px; padding: 6px 14px;")
                    edit_btn.clicked.connect(lambda checked, idx=index: self.nav_list.setCurrentRow(idx))

                    del_btn = QPushButton("🗑 Remove")
                    del_btn.setStyleSheet("background-color: #511; color: #f88; border: none; padding: 6px 14px; border-radius: 4px;")
                ...
                # --- RESPONSIVE GRID FLOW LAYOUT ---
                self.flow_layout = FlowLayout(margin=0, spacing=15)

                for index, mod in enumerate(self.modules, start=1):
                    card_frame = QFrame()
                    card_frame.setFixedSize(270, 160)
                    card_frame.setStyleSheet("""
                        QFrame {
                            background-color: #1e1e1e;
                            border: 1px solid #333338;
                    ...
                    grid_card_layout.setContentsMargins(15, 15, 15, 15)
                    ...
                    grid_btn_layout.setSpacing(6)

                    launch_btn = QPushButton("Launch")
                    launch_btn.setStyleSheet("background-color: #007acc; color: white; border: none; padding: 6px; border-radius: 4px; font-weight: bold;")
                    launch_btn.clicked.connect(lambda checked, path=mod['path']: self.launch_external_persistent(path))

                    edit_btn = QPushButton("Edit")
                    edit_btn.setStyleSheet("background-color: #333337; color: #dcdcdc; border: 1px solid #555; border-radius: 4px; padding: 6px;")
                    edit_btn.clicked.connect(lambda checked, idx=index: self.nav_list.setCurrentRow(idx))

                    del_btn = QPushButton("Remove")
                    del_btn.setStyleSheet("background-color: #511; color: #f88; border: none; padding: 6px; border-radius: 4px;")
                    del_btn.clicked.connect(lambda checked, path=mod['path']: self.remove_module(path))
        ...
        for index, mod in enumerate(self.modules, start=1):
            ...
            header_layout.addWidget(title_info, 1)
